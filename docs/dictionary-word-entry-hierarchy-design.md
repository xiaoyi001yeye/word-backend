...
词书页面的“添加词书”支持通过 AI 补全单词详情。AI 返回的结构化词条必须与现有词书数据的词性格式保持一致，`partOfSpeech[].pos` 使用英语词典缩写，不使用完整英文单词：
...
例如单词 `abandon` 的 AI 词性必须返回 `vt.`，不能返回 `verb`。后端 AI 提示词负责声明并约束该格式，词书手动录入页面直接将接口返回的标准化词性写入词性输入框，避免页面各自解释 AI 的自然语言词性。

该约束适用于 `POST /api/dictionary-words/{dictionaryId}/words/ai-generate` 及其调用的 `MetaWordEntryDtoV2` 数据结构；人工编辑和批量导入仍允许用户按现有词书格式录入其他词性组合。
