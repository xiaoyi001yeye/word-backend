...
- `DELETE /api/study-plans/{id}`：归档学习计划（管理员可删除全部计划，教师只能删除自己创建的计划）
...
- 只能管理自己负责班级的学习计划
- 可以删除（归档）自己创建的学习计划；不能删除其他教师创建的计划
- 只能查看自己学生的学习进度和缺勤情况
- 只能绑定自己可访问的词书
...
学习计划不执行物理删除。由于计划关联学生计划、每日任务、单词进度、学习记录和注意力统计，删除接口采用软删除：将
`study_plans.status` 更新为 `ARCHIVED`，保留学生学习历史和统计数据。

- `DELETE /api/study-plans/{id}` 成功返回 `204 No Content`
- 管理员可以归档任意未归档计划
- 教师只能归档 `teacher_id` 等于当前用户 ID 的计划
- 学习计划列表过滤 `ARCHIVED`，归档后不再出现在管理员或教师列表中
- 学生端只允许访问已发布且未归档的计划；归档不会删除已产生的学习记录
- 归档计划不允许再次编辑、发布、追加学生或查看管理端明细
