...
- `DELETE /api/study-plans/{id}`：删除学习计划，仅允许删除尚未开始的 `DRAFT` 计划（管理员可删除全部草稿计划，教师只能删除自己创建的草稿计划）
...
- 只能管理自己负责班级的学习计划
- 可以删除状态为 `DRAFT`（新建、尚未开始）的自己创建的学习计划；不能删除其他教师创建的计划，也不能删除已发布、暂停或已完成的计划
- 只能查看自己学生的学习进度和缺勤情况
- 只能绑定自己可访问的词书
...
- 可查看全部计划
- 可删除任意教师的 `DRAFT`（新建、尚未开始）学习计划；已开始或已完成的计划同样不能删除
- 可查看全部学习记录
- 可用于运营排查和数据修复

### 16.4 学习计划删除规则

只有“新建、尚未开始”的学习计划允许删除。判定标准是计划状态为 `DRAFT`：`DRAFT` 表示计划已创建但尚未发布，没有生成每日任务，也没有任何学习记录；一旦发布进入 `PUBLISHED`（以及后续的 `PAUSED`、`COMPLETED`），计划就进入“已进行”状态，不再允许删除。

删除是物理删除，且不依赖数据库级联：按序清理 `student_study_plans`、`study_plan_classrooms`，再删除 `study_plans` 行。

- `DELETE /api/study-plans/{id}` 仅对 `DRAFT` 计划生效，成功返回 `204 No Content`
- 非 `DRAFT` 计划返回 `400`，提示 `Only draft study plans that have not started can be deleted`
- 管理员可以删除任意教师的 `DRAFT` 计划
- 教师只能删除 `teacher_id` 等于当前用户 ID 的 `DRAFT` 计划
- 删除前校验该计划的 `student_study_plans` 是否已产生每日任务、学习记录、单词进度或注意力统计：只要存在任意一条，就返回 `400` 并拒绝删除，确保物理删除永远不会销毁学习历史（正常草稿计划不会有这些数据）
- 草稿计划不会产生积分事件（积分只从已完成任务和学习记录派生），也未被分享到班级群动态（分享要求 `PUBLISHED`），因此物理删除不会留下悬空引用
- 学生级数据（`student_word_memories`、积分流水、班级成员关系）与计划无关，不受影响
- 系统不再提供“归档”作为删除的替代出口：`ARCHIVED` 状态和历史归档数据仍保留在库中并继续被列表过滤，但接口不会再产生新的归档计划
- 学习计划列表过滤 `ARCHIVED`；学生端只允许访问已发布且未归档的计划
